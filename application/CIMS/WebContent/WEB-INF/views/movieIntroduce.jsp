	...
	<div class="row">
		...
		<div class="column2">
			<div>
				...
				<div class="movieDiv">
					...
					<p>
						类型：
						<c:forEach items="${types }" var="type">
							<c:if test="${type.type_id eq movie.type_id }">${type.type_name }</c:if>
						</c:forEach>
					</p>
					<br>
					<p>语言：${movie.movie_language }</p>
					<br>
					<p>上映日期：${movie.movie_date }</p>
					<br>
					<p>简介：${movie.movie_introduce }</p>
				</div>
			</div>
			<div>
				<table class="table table-hover">
					...
					<c:forEach items="${scenes }" var="scene">
						<tr>
							<td><c:forEach items="${cinemas }" var="cinema">
									<c:if test="${cinema.cinema_id eq scene.cinema_id }">${cinema.cinema_name }</c:if>
								</c:forEach></td>
							<td>${scene.scene_date }<br> ${scene.start_time }
							</td>
							<td>${scene.scene_date }<br> ${scene.end_time }
							</td>
							<td><c:forEach items="${seats }" var="seat">
									<c:if test="${seat.seat_id eq scene.seat_id }">${seat.seat_name }</c:if>
								</c:forEach></td>
							<td>${scene.price }</td>
							<td><a
								href="${pageContext.request.contextPath}/home/choiceScene?scene_id=${scene.scene_id}">选座购票</a></td>
						</tr>
					</c:forEach>
				</table>
			</div>
			<div class="comment">
				<div>
					<div class="b-head">
						<span class="b-head-t" style="margin-left:55px;font-size:30px">最新评论</span>
					</div>
				</div>
				...
				<div class="comment" style="position: relative;">
					<div class="comment-m">
						...
						<div class="comment-list" style="margin-left:33px">
						      <p class="sty" style="font-size: 10px;">最新评论</p>
				...
				<div class="page">
                <div>
                   <a href="${pageContext.request.contextPath}/home/comment?movie_id=${movie.movie_id}">查看全部评论</a>
                </div>
            </div>
			</div>

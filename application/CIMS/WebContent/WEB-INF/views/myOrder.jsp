...
<div class="row">
  ...
  <div class="column2">
    <table class="table">
        <tr><th>订单编号</th><th>订单时间</th><th>座位</th><th>订单金额</th><th>订单状态</th></tr>
        
        <c:forEach items="${orders }" var="order">
        <tr style="height:80px;" onclick="location.href='${pageContext.request.contextPath}/home/pay?order_id=${order.order_id }';">
        <td>${order.order_id }</td>
        <td>${order.order_time }</td>
        <td>${order.seat_location }</td>
        <td>${order.order_price }</td>
        <td>
        <c:if test="${order.order_state==-1 }">已过期</c:if>
        <c:if test="${order.order_state==0 }">待付款</c:if>
        <c:if test="${order.order_state==1 }">已付款</c:if>
        </td>
        </tr>
        </c:forEach>
        </table>
